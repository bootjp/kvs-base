...
func newCluster(n int) *cluster {
	...
	for i := range clus.peers {
		...
		proposeResp := make(chan bool)
		clus.commitC[i], clus.errorC[i], _ = newRaftNode(i+1, clus.peers, false, fn, clus.proposeC[i], clus.confChangeC[i], proposeResp)
	}
...
func TestProposeAndLoad(t *testing.T) {
	...
	proposeResp := make(chan bool)
	commitC, errorC, snapshotterReady := newRaftNode(1, clusters, false, getSnapshot, proposeC, confChangeC, proposeResp)

	kvs = newKVStore(<-snapshotterReady, proposeC, commitC, errorC, proposeResp)
	<-time.After(time.Second * 3)

	for i := 0; i <= 99999; i++ {
		wantKey, wantValue := "test-key", []byte("test-value"+strconv.Itoa(i))
		kvs.Propose(wantKey, wantValue)
		data, ok := kvs.Lookup(wantKey)
		if gotValue := data; ok && !reflect.DeepEqual(wantValue, gotValue) {
			t.Fatalf("expect %s, got %s", wantValue, gotValue)
		}
		t.Log(string(wantValue), "is check ok")
	}
}
...
func TestAddNewNode(t *testing.T) {
	...
	newRaftNode(4, append(clus.peers, newNodeURL), true, nil, proposeC, confChangeC, proposeResp)
